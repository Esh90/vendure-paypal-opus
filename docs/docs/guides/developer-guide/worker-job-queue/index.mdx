...
Since v3.7.0, the `DefaultJobQueuePlugin` exposes a first-class `rateLimit` option that lets
you say "process at most N jobs per time window" for a given queue. This is useful when a job
handler calls a rate-limited downstream system — for example a transactional email provider
that allows at most 60 messages per hour.
...
The `duration` can be specified as a number of milliseconds or as a shorthand string — `'500ms'`,
`'30s'`, `'5m'`, `'1h'`, `'2d'`.

#### Concurrency vs rate limit

`concurrency` and `rateLimit` solve different problems and can be combined:

- **`concurrency`** caps how many jobs from a given queue run _in parallel_ at any moment.
- **`rateLimit`** caps how many jobs are _started_ per sliding time window, regardless of
  whether earlier jobs have finished.

For example, with `concurrency: 10` and `rateLimit: { max: 60, duration: '1h' }`, up to 10 jobs
can run in parallel, but across any rolling 1-hour window no more than 60 jobs will be started.

#### Multi-worker behavior

When running multiple horizontally-scaled Vendure worker instances against the same database,
the `DefaultJobQueuePlugin` coordinates the rate limit across all workers by counting jobs whose
`startedAt` timestamp falls inside the window. Counting starts — rather than completions — means
that if a worker crashes mid-job, that job still consumes its budget slot until the window rolls
forward, which is the desired shed-load behavior against an unreliable downstream.

Because the cross-worker count is not globally locked, a transient overshoot of up to
`(numWorkers - 1)` jobs per window is possible under heavy contention. If your downstream
requires strict enforcement with zero overshoot, set the rate limit slightly below the
downstream's hard ceiling to leave headroom.

#### Long-running jobs

Because the limiter counts starts, a job that takes longer than `duration` to complete will
have its slot recycled while it is still running. For example, with
`{ max: 1, duration: '1s' }` and a handler that takes 3 seconds, a second job will start at
`t=1s` while the first is still in flight. Set `concurrency: 1` on the same queue if you need
at-most-one-in-flight semantics in addition to a rate limit.

:::note
Rate limiting is not yet supported by the `BullMQJobQueuePlugin`. BullMQ's native worker
`limiter` is queue-global and all Vendure jobs share a single BullMQ queue, so forwarding it
naively would throttle unrelated queues. A BullMQ-specific implementation is tracked as a
follow-up.
:::
